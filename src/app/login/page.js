  ...
  const submitHandler = async (e) => {
    ...
    try {
      const response = await fetch(`${BASE_URL}/login`, {
        ...
        body: JSON.stringify({ usernameOrEmail, password, isMobile: mobileCheck }),
        credentials: "include",
      });
    ...
    <div className="flex justify-center items-center min-h-screen bg-gradient-to-r from-[#FDB439] to-[#FA9D39]">
      <div className="flex flex-col items-center gap-4">
        ...
        <form
          ...
          className="bg-white p-8 rounded-lg shadow-lg w-96 max-w-md flex flex-col items-center"
        >
          <h2 className="text-2xl font-bold text-center text-[#333333] mb-6">Lets ChitChat!</h2>
          <div className="mb-4 w-full">
            <input
              id="usernameOrEmail"
              name="usernameOrEmail"
          ...
          <button
            type="submit"
            disabled={disabled}
            className={`w-full py-3 rounded-md text-white font-semibold ${disabled ? "bg-[#DDA15E]" : "bg-[#FDB439] hover:bg-[#FA9D39]"} transition duration-200`}
          >
            {disabled ? "Signing in..." : "Sign in"}
          </button>
          <div className="mt-4 text-center">
            <Link href="/signup" className="ml-1 text-[#FDB439] hover:text-[#FA9D39] font-semibold text-md transition duration-200">
              Register here
            </Link>
          </div>
        </form>
